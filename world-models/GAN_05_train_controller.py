...
# Command we use
# xvfb-run -a -s "-screen 0 1400x900x24" python GAN_05_train_controller.py car_racing -n 4 -t 2 -e 2 --max_length 500
# xvfb-run -a -s "-screen 0 1400x900x24" python GAN_05_train_controller.py car_racing -n 4 -t 1 -e 1 --max_length 500 --dream_mode 1
...
from GANmodel import make_model, simulate
...
def initialize_settings(sigma_init=0.1, sigma_decay=0.9999, init_opt = ''):
  ...
  population = num_worker * num_worker_trial
  filebase = './log/GAN/'+env_name+'.'+optimizer+'.'+str(num_episode)+'.'+str(population)
  controller_filebase = './controller/GAN/'+env_name+'.'+optimizer+'.'+str(num_episode)+'.'+str(population)
...
if __name__ == "__main__":
  ...
  parser.add_argument('--eval_steps', type=int, default=10, help='evaluate every eval_steps step')
