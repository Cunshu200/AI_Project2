# python GAN_04_train_rnn.py --new_model --batch_size 200
# python GAN_04_train_rnn.py --new_model
# python GAN_04_train_rnn.py
...
ROOT_DIR_NAME = './data/'
SERIES_DIR_NAME = './data/GAN_series/'
...
def main(args):
	...
	if not new_model:
		try:
			rnn.set_weights('./rnn/GAN_weights.h5')
		except:
			print("Either set --new_model or ensure ./rnn/GAN_weights.h5 exists")
			raise
	...
	for step in range(steps):
		...
		if step == 0:
			np.savez_compressed(ROOT_DIR_NAME + 'GAN_rnn_files.npz', rnn_input = rnn_input, rnn_output = rnn_output)

		rnn.train(rnn_input, rnn_output)

		if step % 10 == 0:

			rnn.model.save_weights('./rnn/GAN_weights.h5')
...
if __name__ == "__main__":
		...
		parser.add_argument('--N',default = 3000, help='number of episodes to use to train')
		parser.add_argument('--new_model', action='store_true', help='start a new model from scratch?')
		parser.add_argument('--steps', default = 2000, help='how many rnn batches to train over')
		parser.add_argument('--batch_size', default = 200, help='how many episodes in a batch?')
